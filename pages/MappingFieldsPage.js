 ...
 *
 * Page Object for the Field Mapping step of the Zephyr Essential import
 * wizard — each column from the CSV is mapped to a corresponding Jira/Zephyr
 * field. After mapping, "Next" advances to the Data Mapping step
 * (DataMappingPage), and after that the actual import runs and a Results
 * screen appears.
 */
...
export default class MappingFieldsPage {
  ...
  constructor(page) {
    ...
    // "Next" button that advances from Field Mapping to Data Mapping
   ...
   *   column name and each value is the Zephyr/Jira field to map it to.
   ...
   *  3. Types the desired field name and presses Enter to confirm.
   *
   * After all mappings are set, clicks Next to proceed to Data Mapping.
   */
  async updateFieldMappings(fieldMap) {
    ...
    for (const [columnName, mappingValue] of Object.entries(fieldMap)) {
      ...
      // Type and confirm the field name
      await dropdown.fill(mappingValue);
      await dropdown.press('Enter');
    }

    // Advance to the Data Mapping step
    await this.nextButton.click();
  }

  /**
   * Reads the Results screen after the import completes and extracts the
  ...
  async getTheNumberOfSuccessfullyMappedIssues() {
    // Zephyr Essential's Results screen uses its own success banner rather
    // than the old AUI message classes (.aui-message-success). Fall back to
    // a text-based match on the summary message so this survives minor
    // styling/markup differences.
    ...
    // Extract the number from messages like "42 issues were successfully imported"
    const match = successMessage.match(/(\d+)\s+(?:issues?|work items?|tests?)/i);
  ...
  /**
   * Clicks the link on the Results screen that navigates to the newly
   * created issues (Zephyr Essential's wording may differ slightly from
   * "Check created issues." — this matches loosely on intent).
   *
   * Waits 5 seconds first to ensure the Results screen has fully rendered.
   */
  async clickCheckCreatedIssues() {
    await this.page.waitForTimeout(5000);
    await this.page
      .getByRole('link', { name: /view|check|created issues/i })
      .first()
      .click();
  }
}
